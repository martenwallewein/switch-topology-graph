...
def process_traffic_data_from_folder(results_dir):
    ...
    Now looks for 'result_inflation_X_run_Y.json' files.
    """
    ...
    # Regex handles the new naming convention: result_inflation_1.1_run_1.json
            ...
            try:
                ...
                for block_name in results.keys():
                    ...
                    if total_sent_traffic > 0:
                        ...
                        results[block_name][inflation_factor].append(spillover_ratio)
                    else:
                        results[block_name][inflation_factor].append(0.0)

            except Exception as e:
                print(f"    - Warning: Could not process file '{filename}'. Reason: {e}")

    print(f"    - Found and processed {files_found} files.")
    return results
...
def analyze_and_plot_congestion():
    ...
    showing the spillover ratio against Latency Inflation.
    """
    ...
    # (Matches the output from the previous workflow script)
    scenario = {
        "path": "results/worst_case/with_prefer_peering/results",
    }
    output_plot_file = "congestion_vs_latency_inflation.pdf"
    
    # Plot styles for each analysis block
    plot_configs = {
        "latency_optimal": {
            "label": "Thundering Herd (1 Path)",
            "color": "red",
            "linestyle": "-"
        },
    ...
    if not all_block_results:
        print("\nNo data found in directory. Please check the path.")
        return
    ...
    for block_name, factor_data in all_block_results.items():
        ...
        # Scatter points (background, faint) showing distribution of runs
    ...
    ax.set_xlabel('Latency Inflation Factor (2nd Path)')
    ...
    if all_scenario_factors:
        ...
        # If we have many ticks (e.g. 0.1 steps from 1.0 to 3.0), we might need to filter or rotate
        ax.set_xticks(sorted_list)
        
        # Only rotate if we have a lot of data points
        if len(sorted_list) > 10:
            plt.xticks(rotation=45)
            # Optional: Show only every 2nd tick if it's too crowded
            # ax.set_xticks(sorted_list[::2]) 
