...
def generate_traffic_scenario(graph_data, traffic_df, traffic_increase_factor=1.0, cost_difference_factor=3.5):
    ...
    for iface in egress_interfaces:
        ...
        if iface in transit_links:
            egress_to_destination_reachability[iface_id] = destinations  # Transit reaches all
        else:  # Peering link
            peer_name = iface.get('to')
            mapped_dest_name = GRAPH_TO_CSV_NAME_MAP.get(peer_name, peer_name)
            egress_to_destination_reachability[iface_id] = [mapped_dest_name] if mapped_dest_name in destinations else []

    # 4. Assign latencies and realistic costs (Base and Dynamic)
    egress_latencies = {}
    egress_base_costs = {}
    ...
    # 5. Build the final scenario dictionary
    scenario = {
        "endhosts": endhosts,
