...
def run_workflow():
    ...
    scenario_output_dir = "thundering_herd/scenarios"
    results_output_dir = "thundering_herd/results"
    ...
    # --- 2. Loop through the traffic ratios and repetitions ---
    for traffic_ratio in range(1, 20):  # Loops from 2 to 10
        print(f"\n{'='*50}")
        print(f"--- Processing all runs for traffic_ratio: {traffic_ratio} ---")
        print(f"{'='*50}")
        for run_number in range(1, 50):  # Loops 50 times (from 1 to 50)
            print(f"\n  - Starting run {run_number}/50 for factor {traffic_ratio}...")

            # --- 3. Define unique file paths for this iteration ---
            base_filename = f"factor_{traffic_ratio}_run_{run_number}"
