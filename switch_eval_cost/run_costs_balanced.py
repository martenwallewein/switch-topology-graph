...
def run_workflow():
    """
    Orchestrates a two-step process for multiple configurations:
    1. Calls scenario_gen_csv.py to generate scenario files, with and without
       the --prefer_peering flag.
    2. Calls run_all_scenarios.py using each generated scenario file.
    """
    # --- Configuration ---
    ...
    # --- Define the different experiment configurations to run ---
    configurations = [
        {
            "name": "without_prefer_peering",
            "use_prefer_peering": False,
            "scenario_dir": "peering_transit_factor_balanced_without_prefer_peering/scenarios",
            "results_dir": "peering_transit_factor_balanced_without_prefer_peering/results"
        },
        {
            "name": "with_prefer_peering",
            "use_prefer_peering": True,
            "scenario_dir": "peering_transit_factor_balanced_with_prefer_peering/scenarios",
            "results_dir": "peering_transit_factor_balanced_with_prefer_peering/results"
        }
    ]
    ...
    for config in configurations:
        ...
        scenario_output_dir = config["scenario_dir"]
        results_output_dir = config["results_dir"]
        use_prefer_peering = config["use_prefer_peering"]

        # --- 1. Create the output directories for the current configuration ---
        try:
        ...
        for cost_factor in range(2, 21):  # Loops from 2 to 20
            ...
            for run_number in range(1, 11):  # Loops 10 times (from 1 to 10)
                ...
                # *** MODIFICATION: Conditionally add the --prefer_peering flag ***
                if use_prefer_peering:
                    generator_command.append("--prefer_peering")
                    print("    (Using --prefer_peering)")
